...
int main(){
 ...
 {markup::Options options;std::vector<std::string>d;options.rst_diagnostic=[&](const std::string&v){d.push_back(v);};std::string a,b;if(!markup::convert(markup::Format::ReStructuredText,".. raw:: html\n\n   <script>x</script>\n",a,b,options)||!a.empty()||d.size()!=1)return 7;++checks;}
 {markup::Options options;options.standalone=true;options.title="Safe <document>";std::string a,b;if(!markup::convert(markup::Format::ReStructuredText,"Body.",a,b,options)||a.find("<title>Safe &lt;document&gt;</title>")==std::string::npos)return 8;++checks;}
